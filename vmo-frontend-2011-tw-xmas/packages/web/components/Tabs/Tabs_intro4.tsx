...
import { Tabsintro4TimeState, Iframes, TrackTab } from '@vmo/web/constants/config';
...
const Tabs_intro4 = () => {
  ...
  useEffect(() => {
    trackingSource.track(
      createTabClickAction(
        '',
        `${translation.get('TAB_3')}/${translation.get(
          'EVENT_INTRO_TITLE',
  ...
  const Tabs_intro4Items = [
    {
        tabKey: Tabsintro4TimeState[0].tabKey,
      ...
      {
        tabKey: Tabsintro4TimeState[1].tabKey,
      ...
      {
        tabKey: Tabsintro4TimeState[2].tabKey,
  ...
  const defaultTabIndex = getDefaultDay(Tabsintro4TimeState, 0);
  return (
    
    <Tabs_intro1Styled active={Tabsintro4TimeState[defaultTabIndex].tabKey}>
      {Tabs_intro4Items.map(item => (
        <Tab
